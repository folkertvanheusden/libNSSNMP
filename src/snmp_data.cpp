...
static const uint64_t running_since = get_micros();
...
{
	uint64_t now = (get_micros() - ::running_since) / 10000;

	return new snmp_integer(snmp_integer::si_ticks, now - this->running_since);  // 100ths of a second
}
